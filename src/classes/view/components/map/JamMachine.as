package classes.view.components.map
{
    ...
    public class JamMachine extends MultiProcessor
    {
        ...
        public function JamMachine(value:Object)
        {
            work_anim = [[1, 68]];
        ...
        override protected function queue_highlight_by_raw_material(value:Number) : void
        {
            if (value == 0)
            {
                return super.queue_highlight_by_raw_material(value);
            }
            if (process_count["feed" + (value + 1)] > 0)
            {
                mc.honey_levels.visible = true;
                mc.honey_levels.gotoAndStop(process_count["feed" + (value + 1)] + raw_materials[value].length);
        ...
        {
            var _loc_1:* = next_stage_in() * 1000 - 1000;// 这２段代码怎么会一样呢
        ...
        private function prepare_honey_anim() : void
        {
            ...
            valve_opened = true;
            var temp:int = raw_materials[1].length - 1;
            honey_player.play([[TimelinePlayer.CURRENT_FRAME, temp * 10]]);
            honey_updated = true;
        }

        override protected function update_raw_material(value:Number) : void
        {
            if (value == 0)
            {
                return super.update_raw_material(value);
            }
            if (!mc)
            {
                return;
            ...
            if (!honey_updated)
            {
                ...
                if (raw_materials[value].length > 0)
                {
                    honey_player.play([[TimelinePlayer.CURRENT_FRAME, raw_materials[value].length * 10]]);
                }
                else
                {
        ...
        override public function set_raw_material(value:Number) : void
        {
            super.set_raw_material(value);
            if (next_product_in() <= 0)
            {
                if (mc)
                {
                    mc.anim.funnels.gotoAndStop(value + 1);
                }
            }
        }
