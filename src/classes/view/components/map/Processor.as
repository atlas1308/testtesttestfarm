package classes.view.components.map
{
    ...
    public class Processor extends CollectObject implements IProcessor
    {
        ...
        public function Processor(value:Object)
        {
            ...
            animals = Math.max(1,animals);// 最小值为1
		 ...
		 * 检验产物的方法
		 * 后台根据这个计算即可
		 * 这里的collec_in是和当前的动物的数量有关系的
		 * Math.min(raw_materials, Math.floor((Algo.time() - start_time) / collect_in)
		 * 以上这段代码计算的是消耗的原料数,通过经过的时间/collect_in来计算当前的消耗
		 * max_products - products,计处出还有多少个product可以产出,因为有最大的max_products限制
		 * 如果还有product可以产出,并且还有原料的话,取最小一个进行生产,
		 * 也就是说,如果消耗的原料和产生物,不相同的话,那就说明并且取的最小值大于1的话,就说明生产了1个,进行计算
		 * 然后就是判断最大的产物了
		 */ 
        protected function check_products() : void
        {
            ...
            if (start_time)// 这个判断可能多余了,上面这段代码应该去掉
            {
                ...
                index = Math.min(max_products - products, Math.min(raw_materials, Math.floor(diffTime / collect_in)));
                if (index < 0)
                {
                    index = 0;
                }
        ...
        public function refill_over() : Boolean
        {
            ...
            if (refill_area)// 这里就是几个geom下的具体应用了
            {
