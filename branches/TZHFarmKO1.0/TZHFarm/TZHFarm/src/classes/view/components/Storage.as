package classes.view.components
{
    ...
    public class Storage extends Sprite implements IChildren
    {
        ...
        private var max_items:Number = 12;
        ...
        private function view_page(value:Number):void
        {
            ...
            var _loc_2:* = value * max_items;
            var _loc_3:* = Math.min(items.length, _loc_2 + max_items);
            var _loc_5:* = _loc_2;
            while (_loc_5 < _loc_3)
            {
                
                storageItemRender = new StorageItem(items[_loc_5]);
                container.addChild(storageItemRender);
                storageItemRender.x = (_loc_5 - _loc_2) % 3 * (storageItemRender.width + 15);
                storageItemRender.y = int((_loc_5 - _loc_2) / 3) * (storageItemRender.height + 10);
                storageItemRender.addEventListener(storageItemRender.ON_SELL, sellItem);
                _loc_4 = storageItemRender.width;
                _loc_5++;
            }
            _loc_6 = _loc_4 * 3 + 30;
