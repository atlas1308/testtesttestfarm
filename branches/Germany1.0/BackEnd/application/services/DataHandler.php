...
class DataHandler
{
    ...
    protected $support_langs = array('en-US', 'ko-KR');     //支持的语言包
	...
	*/
	private function retrieveData($channel = null)
	{
		...
		$ret['config']->store = $store->getData($this->lang);
		unset($store);
		...
		if ($userinfo) {
			...
			//判断是否升级
			$upgrade = $this->checkUserLevel($this->uid, $userinfo->level, $userinfo->experience);
			$ret['data']->level = $upgrade['new_level'];					
			
			/*
			 * 如果升级，组织一个对象输出 level_up.coins,level_up.rp
			 */
			if($upgrade['is_upgrade']) {
				$level_up = (Object)array(
	        	   'coins'  => $userinfo->coins,
	        	   'rp'  => $userinfo->reward_points,
				);
				$ret['data']->level_up = $level_up;
			}
			//update by dabin 20100829  
				...
				foreach ($this->input->queue as $queue) {
					...
					switch ($queue->method) {
						...
						case 'add_object':
							...
							//TODO 开放下面这句debug 添加地块的时候系统会报500错误 20100829 by dabin
							...
							// 更新用户信息
							$user = new UserModel();
							//TODO  by dabin  $reward_points 不能小于0 临时为前台处理，未解决问题
							$reward_points = (0 - $rp);
						...
						case 'collect_product':
							...
							// TODO 如果是动物或鸡圈
							$row = $store->getProductsById($queue->data->id);
							...
							//if($row->type == "trees") {
							if(isset($row->type)&&$row->type == "trees") { //@TODO by dabin  20100830
								$map = new MapModel();
							...
							if(isset($row->type)&&$row->type != 'seeds') break; //@TODO by dabin  20100830
	...
	private function getMapItemSellPrice($uid, $map_x, $map_y)
	{
		...
		//TODO 临时处理，这个方法的调用导致  删除地块出问题 update  by dabin 20100829
		$row = $store->getProductsById($item->id);
	...
	private function checkUserLevel($uid, $level, $experience)
	{
		$upgrade = array(
			'is_upgrade' => false,
			'new_level' => $level
		); //以数组返回 便于判断是否升级  update by dabin 20100829
