 ...
 * @package    Ko
 * @author     Ko Team, Eric
 * @version    $Id: core.php 103 2010-08-12 06:52:18Z eric $
...
class Ko
{
    ...
    public static function config ($group)
    {
        ...
        if (! isset($config[$group])) {
            ...
            $config[$group] = Config::instance(new Config_File)->load($group);
        }
        if (isset($path)) {
    ...
    /**
     * Creates a new Language object for the requested group.
     *
     * @param   string   group name
     * @return  Config
     */
    public static function lang ($group, $lang)
    {
        static $language;
        
        $lang = str_replace(array(' ', '_'), '-', $lang);
        if (! isset($language[$lang][$group])) {
            // Load the config lang into the cache
            $language[$lang][$group] = Config::instance(new Config_File('i18n'))->load($lang . DIRECTORY_SEPARATOR . $group);
        }
        return $language[$lang][$group];
    }
