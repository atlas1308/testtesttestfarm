package classes.view
{
    ...
    public class PopupMediator extends Mediator implements IMediator
    {
        ...
        private function onAccept(event:Event) : void
        {
            ...
            {
                Log.add("refresh page popup accept");
                sendNotification(ApplicationFacade.NAVIGATE_TO_URL, "");
            }
            if (popup as CashPopup)
            {
                app_data.navigate_to("offers");
            }
            if (popup as LevelUpPopup)
            {
                if (LevelUpPopup(popup).is_valid())
                {
                    app_data.show_feed_dialog();
                }
            }
            ...
            if (popup as DynamicPopup)
            {
                switch(DynamicPopup(popup).type)
                {
                    ...
                    case PopupTypes.ACCEPT_SELECTED_GIFT:
                    {
                        onClose(null);
                    ...
                    {
                        app_data.post_on_friend_wall(DynamicPopup(popup).info.neighbor);
                        break;
                    }
            ...
            if (popup as DynamicPopup)
            {
                switch(DynamicPopup(popup).type)
                {
                    ...
                    case PopupTypes.SELECT_RAW_MATERIAL:
                    {
                        ...
                        {
                            app_data.report_confirm_error("No " + SelectPopup(popup).selected_name + " in the Barn!");
                        }
                        break;
                    }
                    default:
        ...
        {
            onClose(event);
            var _loc_2:Object = {neighbor:NeighborsListPopup(event.target).selected_neighbor, gift:event.target.info.gift};
            var _loc_3:* = app_data.get_accept_selected_gift_popup_data(_loc_2.neighbor, _loc_2.gift);
            _loc_3.data = _loc_2;
            sendNotification(ApplicationFacade.SHOW_POPUP, _loc_3);
            return;
        }
