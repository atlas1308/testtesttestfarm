package classes.view.components.shop
{
    ...
    public class ShopItem extends Sprite implements ISkinProperties
    {
        ...
        {
            /* var gameButton:GameButton = event.target.parent as GameButton;
            if (gameButton && gameButton == buy_btn || gameButton == buy_rp_btn || gameButton == send_gift)
            {
        ...
        private function init():void
        {
            ...
            if (data.price)
            {
                cost.text = Algo.number_format(data.price) + " coins";
            }
            else
            {
                cost.text = String(data.rp_price) + " RC";
                /* if (!data.locked && data.buy_gift)// 暂时先关闭这个功能
                {
                    send_gift = new GameButton("SEND", 13, 15 / 13);
                    send_gift.set_style("blue");
                    addChild(send_gift);
                    _loc_1 = (width - buy_rp_btn.width - send_gift.width) / 3;
                    buy_rp_btn.x = _loc_1;
                    send_gift.x = buy_rp_btn.x + buy_rp_btn.width + _loc_1;
