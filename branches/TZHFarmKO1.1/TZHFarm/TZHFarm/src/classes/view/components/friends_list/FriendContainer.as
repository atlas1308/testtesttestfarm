package classes.view.components.friends_list
{
    ...
    public class FriendContainer extends MovieClip
    {
        ...
        {
            this.createChildren();
            this.data = JSDataManager.getInstance().merge(value);
            this.init();
        }
        ...
        private function photoComplete(event:Event) : void
        {
            ...
            photo_mask.addChild(bitmap);
            bitmap.x = (ww - bitmap.width) / 2;
            bitmap.y = (hh - bitmap.height) / 2;
        ...
        private function init() : void
        {
            ...
            friend_name.autoSize = TextFieldAutoSize.LEFT;
            level.text = String(data.level);
            exp.text = String(data.exp);
            var loader:Loader = new Loader();
            var context:LoaderContext = new LoaderContext();
            loader.load(new URLRequest(String(data.pic)), context);
            loader.contentLoaderInfo.addEventListener(Event.COMPLETE, photoComplete);
            // solve the problem about IOErrorEvent added by feipang
