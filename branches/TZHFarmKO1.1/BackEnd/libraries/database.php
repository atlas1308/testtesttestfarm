 ...
 * @package    Core
 * @author     Ko Team
 * @version    $Id: database.php 91 2010-07-04 03:34:02Z eric $
...
class Database
{
        ...
        foreach ($column as $val) {
            ...
            if (strpos($val, '(') === FALSE and $val !== '*') {
                if (preg_match('/^DISTINCT\s++(.+)$/i', $val, $matches)) {
                    $val = (strpos($matches[1], '.') !== FALSE) ? $this->_config['table_prefix'] . $matches[1] : $matches[1];
                    $this->_distinct = TRUE;
                } else {
                    $val = (strpos($val, '.') !== FALSE) ? $this->_config['table_prefix'] . $val : $val;
                }
    ...
    public function join ($table, $key, $value = NULL, $type = '')
    {
        ...
        foreach ($keys as $key => $value) {
            $key = (strpos($key, '.') !== FALSE) ? $this->_config['table_prefix'] . $key : $key;
            if (is_string($value)) {
    ...
    public function like ($field, $match = '', $auto = TRUE)
    {
        ...
        foreach ($fields as $field => $match) {
            $field = (strpos($field, '.') !== FALSE) ? $this->_config['table_prefix'] . $field : $field;
    ...
    public function in ($field, $values, $not = FALSE)
    {
        ...
        $where = $this->escapeColumn(((strpos($field, '.') !== FALSE) ? $this->_config['table_prefix'] : '') . $field) . ' ' . ($not === TRUE ? 'NOT ' : '') . 'IN (' . $values . ')';
    ...
    public function group ($by)
    {
        ...
        foreach ($by as $val) {
            ...
            if ($val != '') {
                if (strpos($val, '.')) {
                    $val = $this->_config['table_prefix'] . $val;
                }
