package classes.view.components.map {
    ...
    public class CollectObject extends MapObject {
        ...
        protected var is_working:Boolean = false;
        ...
        public function CollectObject(data:Object){
            collect_in = data.collect_in;// 收集的时间,以秒为单位
            ...
            if(_automatic){
            	trace("_automatic" + _automatic + " _product_name " + _product_name);
            }
            super(data);
        }
        ...
        protected function startTimer():void{
            if (next_stage_in() > 0){
                is_working = true;
                interval = setTimeout(check_stage, (next_stage_in() * 1000));
            };
        }
        ...
        public function next_product_in():Number{
            return (Math.max(0, ((start_time + collect_in) - Algo.time())));
        }
